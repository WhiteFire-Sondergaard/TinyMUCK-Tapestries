...
prim_setprop(PRIM_PROTOTYPE)
{
	...
	switch(oper1->type) {
	  case PROG_STRING:
	    result = (int)(oper1->data.string ? oper1->data.string->data : 0);
	    set_property(oper3->data.objref, tname, PROP_STRTYP, result);
	    break;
	  case PROG_INTEGER:
	    result = oper1->data.number;
	    set_property(oper3->data.objref, tname, PROP_INTTYP, result);
	    break;
	  case PROG_OBJECT:
	    ref = oper1->data.objref;
	    set_property(oper3->data.objref, tname, PROP_REFTYP, ref);
	    break;
	  case PROG_LOCK:
	    result = (int)copy_bool(oper1->data.lock);
	    set_property(oper3->data.objref, tname, PROP_LOKTYP, result);
	    break;
	}
