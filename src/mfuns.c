...
mfn_prop(MFUNARGS)
{
    ...
    pname = argv[0];
    if (argc == 2) {
        obj = mesg_dbref_read(player, what, perms, argv[1]);
    }
...
mfn_store(MFUNARGS)
{
    ...
    ptr = argv[0];
    if (argc > 2) {
        obj = mesg_dbref_write(player, what, perms, argv[2]);
    }
    if (obj == UNKNOWN || obj == AMBIGUOUS || obj == NOTHING || obj == HOME)
...
mfn_delprop(MFUNARGS)
{
    ...
    pname = argv[0];
    if (argc > 1) {
        obj = mesg_dbref_write(player, what, perms, argv[1]);
    }
    if (obj == UNKNOWN || obj == AMBIGUOUS || obj == NOTHING || obj == HOME)
...
mfn_exec(MFUNARGS)
{
    ...
    pname = argv[0];
    if (argc == 2) {
        obj = mesg_dbref_exec(player, what, perms, argv[1]);
    }
    if (obj == UNKNOWN || obj == AMBIGUOUS || obj == NOTHING || obj == HOME)
    ...
    if (Prop_ReadOnly(pname) || Prop_Private(pname) || Prop_SeeOnly(pname))
        trg = obj;
    ptr = mesg_parse(player, obj, trg, ptr, buf, BUFFER_LEN, mesgtyp);
	    ...
	    if (flag) {
                if (
                       (
                           (int)(endbuf - buf)
                           + strlen(ptr)
                           + 2 /* CR and trailing NULL */
                       )
                       > MAX_MFUN_LIST_LEN
                   )
                {
                    ABORT_MPI("LISTPROPS","Buffer overflow.");
                }
...
mfn_select(MFUNARGS)
{
    ...
    pname = argv[1];
    if (argc == 3) {
        obj = mesg_dbref_read(player, what, perms, argv[2]);
    }
    if (obj == PERMDENIED)
...
mfn_list(MFUNARGS)
{
    ...
    if (!ptr) ABORT_MPI("LIST","Failed list read.");
    return ptr;
}
...
mfn_lexec(MFUNARGS)
{
    ...
    if (!ptr) ABORT_MPI("LEXEC","Failed list read.");
    trg = what;
    if (Prop_ReadOnly(pname) || Prop_Private(pname) || Prop_SeeOnly(pname))
        trg = obj;
    ptr = mesg_parse(player, obj, trg, ptr, buf, BUFFER_LEN, mesgtyp);
...
mfn_timesub(MFUNARGS)
{
    ...
    pname = argv[2];
    if (argc == 4) {
        obj = mesg_dbref_read(player, what, perms, argv[3]);
    }
    if (obj == PERMDENIED)
...
mfn_pronouns(MFUNARGS)
{
    dbref obj = player;
    if (argc > 1) {
	obj = mesg_dbref_proximity(player, what, perms, argv[1]);
	if (obj == UNKNOWN || obj == AMBIGUOUS || obj == NOTHING || obj == HOME)
...
mfn_contains(MFUNARGS)
{
    ...
    dbref obj1 = player;
    if (argc > 1)
	obj1 = mesg_dbref_proximity(player, what, perms, argv[1]);
...
mfn_holds(MFUNARGS)
{
    dbref obj1 = mesg_dbref_proximity(player, what, perms, argv[0]);
    dbref obj2 = player;
    if (argc > 1)
	obj2 = mesg_dbref_proximity(player, what, perms, argv[1]);
...
mfn_loc(MFUNARGS)
{
    ...
    obj = mesg_dbref_proximity(player, what, perms, argv[0]);
    if (obj == UNKNOWN || obj == AMBIGUOUS || obj == NOTHING || obj == HOME)
...
mfn_money(MFUNARGS)
{
    dbref obj = mesg_dbref_read(player, what, perms, argv[0]);
    if (obj == UNKNOWN || obj == AMBIGUOUS || obj == NOTHING || obj == HOME)
...
mfn_otell(MFUNARGS)
{
    ...
    if (obj == PERMDENIED || !(Wizperms(perms) || controls_nowizperm(OWNER(perms), obj)))
        ABORT_MPI("OTELL","Permission denied.");

    /* Listeners can only broadcast to rooms */
    if ((mesgtyp & MPI_ISLISTENER) && (Typeof(what) != TYPE_ROOM))
        ABORT_MPI("OTELL","Permission denied.");

    /* Get a player not to tell to */
    if (argc > 2)
...
mfn_created(MFUNARGS)
{
    ...
    obj = mesg_dbref_read(player, what, perms, argv[0]);
    if (obj == UNKNOWN || obj == AMBIGUOUS || obj == NOTHING || obj == HOME)
