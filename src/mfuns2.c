...
mfn_locked(MFUNARGS)
{
    dbref who = mesg_dbref_proximity(player, what, perms, argv[0]);
    dbref obj = mesg_dbref_read(player, what, perms, argv[1]);
...
mfn_testlock(MFUNARGS)
{
    struct boolexp *lok;
    dbref who = player;
    dbref obj = mesg_dbref_exec(player, what, perms, argv[0]);

    if (argc > 2)
	who = mesg_dbref_proximity(player, what, perms, argv[2]);
    if (who == AMBIGUOUS || who == UNKNOWN || who == NOTHING || who == HOME)
