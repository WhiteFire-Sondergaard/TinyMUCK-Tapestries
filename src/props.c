...
static PropPtr 
pp_delete(char *key, PropPtr avl)
{
    PropPtr    save;
...
/* copies properties */
void 
copy_proplist(dbref obj, PropPtr *new_list, PropPtr old)
{
    PropPtr p;
	...
	propfetch(obj, old);
#endif
	p = new_prop(new_list, PropName(old));
	...
	copy_proplist(obj, new_list, AVL_LF(old));
	copy_proplist(obj, new_list, AVL_RT(old));
    }
}
